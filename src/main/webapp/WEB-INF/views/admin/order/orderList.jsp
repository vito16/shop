        ...
        <table class="table table-responsive table-striped">
                ...
                <c:forEach items="${page.result}" var="order">
                    <tr pid="${order.id}">
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${order.status==0}"><a class="btn btn-info btn-xs" href="${ctx}/order/view/${order.id}">查看</a> <a class="btn btn-info btn-xs shipping-btn">发货</a> <a class="btn btn-info btn-xs cancel-order-btn">取消订单</a></c:when>
                                <c:when test="${order.status==1}"><a class="btn btn-info btn-xs" href="${ctx}/order/view/${order.id}">查看</a> <a class="btn btn-info btn-xs shipping-btn">发货</a> <a class="btn btn-info btn-xs cancel-order-btn">取消订单</a></c:when>
